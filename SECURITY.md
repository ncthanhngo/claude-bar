...
- **Local-first.** Auth tokens and account configs stay in the macOS Keychain on your Mac.
- **Opt-in cloud.** Nothing leaves your Mac for sync or updates unless you explicitly enable it.
...
| Legacy MCP connector tokens (from builds that shipped the removed Local MCP feature) | Left untouched in the macOS Keychain and carried in the encrypted iCloud bundle so older Macs keep them; no current feature reads them. |
...
| iCloud Drive (`ubiquity` container) | Sync the encrypted account bundle across Macs sharing the same Apple ID. | Only after you click **Enable sync** and enter a passphrase. Background sync every ~6h once enabled. |
...
If the Mermaid diagram doesn't render, the prose summary is: **User → App → Backend (`csw`).** Backend talks to Keychain (always), Anthropic (for usage), iCloud + Sparkle (only when opted in).
